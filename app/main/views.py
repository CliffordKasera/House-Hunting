...
@main.route('/listing/<int:id>', methods = ['GET','POST'])
def single_listing(id):
   ...
   if form.validate_on_submit():
    ...
    # initializing instances
       ...
       #listing instances
       new_booking = Booking(name = name, email = email, contact = contact)
       # save bookings
       new_booking.save_booking()

        return redirect(url_for('singleListing'), id=id)


   title = f'{listing.title} listing'
   return render_template('single-listing.html', title = title, listing_form = form, single_listing = single_listing)
